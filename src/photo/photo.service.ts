...
export class PhotoService {
    ...
    async createOne(createPhotoDto: CreatePhotoDto): Promise<Photo> {
        ...
        //buscar autor por id
        const author = await this.authorRepository.findOneBy({ id: createPhotoDto.authorId });
        if (!author) {
            throw new NotFoundException(`Author with id ${createPhotoDto.authorId} not found`);
        }
    ...
    async updateOne(id: number, updatePhotoDto: UpdatePhotoDto) {
        ...
        if (updatePhotoDto.authorId) {
            const author = await this.authorRepository.findOneBy({ id: updatePhotoDto.authorId });
            if (!author) {
                throw new NotFoundException(`Author with id ${updatePhotoDto.authorId} not found`);
            }
            photo.author = author;
        }
